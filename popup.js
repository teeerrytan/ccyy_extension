...
function def(abc, ghi, jkl, rst, uvw) {
  abc.addEventListener(
    ...
    function() {
      ...
      request.open(
        "POST",
        "http://1.vocabulary1234.applinzi.com/vocabulary1234/1/PhpProject15/new.php",
        true
      );
  ...
  abc.addEventListener(
    ...
    function() {
      ...
      chrome.tabs.sendMessage(
        opq,
        { message: "remove", words: ghi, greeting: "again" },
        function(response_2) {}
      );
      request.open(
        "POST",
...
function search(def) {
  ...
  request_2.open(
    "POST",
    "http://1.vocabulary1234.applinzi.com/vocabulary1234/1/PhpProject15/search.php",
    true
  );
...
chrome.tabs.query(
  ...
  function(tabs) {
    chrome.cookies.get(
      {
        url:
          "http://1.vocabulary1234.applinzi.com/vocabulary1234/1/PhpProject15/signup.php",
        name: "username"
      },
      function(username) {
        if (username) {
          ...
          chrome.storage.sync.get("para_setting", function(para) {
            ...
            if (para.para_setting) {
              ...
              port.onMessage.addListener(function(response) {
                xmlhttp.open(
                  "POST",
                  "http://1.vocabulary1234.applinzi.com/vocabulary1234/1/PhpProject15/index_test.php",
                  true
                );
          ...
          document
            ...
            .addEventListener("click", function() {
              ...
              request.open(
                "POST",
                "http://1.vocabulary1234.applinzi.com/vocabulary1234/1/PhpProject15/bookmark.php",
                true
              );
              request.setRequestHeader("Content-type", "application/json");
          ...
          chrome.tabs.create(
            {
              url:
                "http://1.vocabulary1234.applinzi.com/vocabulary1234/1/PhpProject15/login.php"
            },
            function(tab) {}
          );
        }
